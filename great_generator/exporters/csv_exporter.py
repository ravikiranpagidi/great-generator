...
from great_generator.exporters.pathing import local_pandas_base_path, spark_table_path
...
def export_csv(
    ...
    if engine == "pandas":
        ...
        for table_name, frame in data.items():
            ...
            frame.to_csv(table_path / f"{table_name}.csv", index=False)
        return

    for table_name, frame in data.items():
        frame = prepare_spark_frame(frame, num_partitions, partition_strategy)
        writer = apply_writer_options(frame.write.mode(mode).option("header", True), writer_options)
        writer.csv(spark_table_path(output_path, table_name))
